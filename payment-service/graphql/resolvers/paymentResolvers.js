...
const paymentResolvers = {
    ...
    Payment: {
        __resolveReference: async (paymentRef) => {
            const [rows] = await pool.query('SELECT * FROM payments WHERE id = ?', [paymentRef.id]);
            return rows.length > 0 ? rows[0] : null;
        },
        order: (payment) => ({ __typename: 'Order', id: payment.order_id }),
    },
